$(document).ready(function(){
		...
		$.ajax({
			url: "http://www.omdbapi.com/?s="+search_txt,
		  	...
		  	success:function(data){
		  		...
		  		{
		  			$('#loading ul').append('<li><span>Title: </span><span>'+data.Search[i].Title+'</span><br><span>Year: </span><span>'+data.Search[i].Year+'</span><br><span>Type: </span>'+data.Search[i].Type+'</li>');
		  		}
		  	}
		})
	...
	$(document).delegate('#loading ul li', 'click', function(){
		...
		var year = $(this).find('span:eq(3)').text();
		//console.log(title);
		$.ajax({
			...
			success: function(data) {
				...
				$('#loading').append('<img alt="Poster Not Available" src='+data.Poster+'><br>');
